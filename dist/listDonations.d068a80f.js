...
async function fetchDonations() {
    try {
        const getDonations = (0, _apiConfig.apiClient).get("/dashboard/donations"); // Ensure this endpoint is correct
...
async function initializeDynamicDonationList(containerSelector) {
    ...
    list.rowRenderer(({ rowData, rowElement })=>{
        ...
        const campaignImage = new (0, _image.WFImage)(donationCard.getChildAsComponent("#cardCampaignImage") // Using ID selector
        .getElement());
        if (rowData.campaign_image_url) campaignImage.setImage(rowData.campaign_image_url);
        ...
        const subtitle = rowData.keep_anonymous ? "Anonymous Donation" : `Thank you, ${rowData.customer_first_name}!`;
