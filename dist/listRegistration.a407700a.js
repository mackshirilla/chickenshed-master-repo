...
async function initializeDynamicSubscriptionList(containerSelector) {
    ...
    list.rowRenderer(({ rowData, rowElement })=>{
        ...
        const registrationImage = new (0, _image.WFImage)(registrationCard.getChildAsComponent("#cardRegistrationImage") // Using ID selector
        .getElement());
        ...
        const programName = registrationCard.getChildAsComponent("#cardProgramName"); // Using ID selector
    ...
    try {
        ...
        subscriptions.forEach((sub)=>{
            if (!uniqueSubscriptionsMap.has(sub.program_id)) uniqueSubscriptionsMap.set(sub.program_id, sub);
        });
        const uniqueSubscriptions = Array.from(uniqueSubscriptionsMap.values());
        // Sort subscriptions alphabetically by program name
        uniqueSubscriptions.sort((a, b)=>a.program_name.localeCompare(b.program_name));
