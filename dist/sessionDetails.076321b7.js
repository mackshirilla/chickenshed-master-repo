...
async function initializeSessionDetails() {
    ...
    if (!sessionDetails) return; // Already handled error or navigation
...
function populateWorkshopDetails(workshop) {
    const workshopName = new (0, _core.WFComponent)("#workshopName");
    workshopName.setText(workshop.Name);
    const workshopShortDescription = new (0, _core.WFComponent)("#workshopShortDescription");
...
function populateSessionDetails(session, location) {
    const sessionWeekday = new (0, _core.WFComponent)("#sessionWeekday");
    sessionWeekday.setText(session.Weekday);
    const sessionTime = new (0, _core.WFComponent)("#sessionTime");
    sessionTime.setText(session.Time_block);
    const sessionLocation = new (0, _core.WFComponent)("#sessionLocation");
    sessionLocation.setText(location.Name);
    const sessionLocationMap = new (0, _core.WFComponent)("#sessionLocationMap");
    sessionLocationMap.setHTML(location.Map_embed);
    // Optional: Adjust styling for the embed
    const figureElement = document.querySelector("#sessionLocationMap figure");
    const iframeElement = document.querySelector("#sessionLocationMap iframe");
    const firstChildDiv = document.querySelector("#sessionLocationMap figure > div");
    if (figureElement) {
...
function updateBreadcrumbs(program, workshop, session, subscriptionId, workshopId, caregiver) {
    ...
    if (workshopBreadcrumbElement) {
        ...
        if (workshop) workshopBreadcrumb.setText(workshop.Name);
        else workshopBreadcrumb.setText(program.name);
...
function initializeRemoveStudentDialog(subscriptionId, sessionId, students, caregiver) {
    if (caregiver) // If caregiver, no remove dialog
    return;
